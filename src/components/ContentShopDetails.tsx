...
const ContentShopDetails = () => {
  ...
  } = useFetchShopDetails(Number(id));

  if (errorShopdetails) {
    return (
      <p className="flex items-center justify-center bg-black text-text_default">
        {errorShopdetails}
      </p>
    );
  }
      ...
      <main className="p-8 md:h-screen md:overflow-y-auto bg-black text-white">
        <div className="md:p-8 flex flex-col gap-10 font-Chillax">
          <div className="flex flex-col gap-3">
            ...
            <p className="text-text_muted text-lg/10 ">
              {shopDetails.description}
            </p>
          </div>

          <div className="flex flex-col gap-3">
            <p className="text-text_default text-2xl tracking-wider font-normal">
              $ {shopDetails.price} USD
            </p>
            ...
            <div>
              <div className="grid grid-cols-1 md:grid-cols-2">
                <InputFeild
                  ...
                  className="focus:border-border_white"
                  label="Quantity"
                  error=""
                />
                <DropDowenFeild
                  label="color"
                  placeholder="Color"
                  className="focus:border-border_primary"
                  value={option}
                  onChange={(e) => {
                    setOption(e);
                  }}
          ...
          <div className="flex flex-col gap-3">
            <p className="text-text_primary text-2xl tracking-wider font-normal mb-7">
              Details
            </p>
            <p className="text-text_muted text-lg/9 mb-4">
              {shopDetails.details}
            </p>
          </div>
        </div>
